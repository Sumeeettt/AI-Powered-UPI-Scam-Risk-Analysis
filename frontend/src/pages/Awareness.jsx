    ...
    <div className="min-h-screen bg-page text-ink">
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 md:py-16">
        <header className="max-w-3xl">
          ...
          <h1 className="mt-5 text-4xl font-bold tracking-tight text-ink md:text-5xl">UPI Fraud Awareness</h1>
        ...
        <section className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {cards.map(({ title, description, icon: Icon, tone }) => (
            <article key={title} className="rounded-2xl border border-slate-700/50 bg-panel p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primaryBlue/30 hover:shadow-md">
        ...
        <section className="mt-12 rounded-2xl border border-slate-700/50 bg-panel p-6 shadow-soft">
          <h2 className="text-2xl font-bold text-ink">What to do if you are scammed?</h2>
          <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {emergencySteps.map((step, index) => (
              <div key={step} className="rounded-xl border border-slate-700/50 bg-panel p-4">
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primaryBlue text-sm font-bold text-white">{index + 1}</div>
                <p className="mt-4 font-semibold leading-6 text-ink">{step}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="mt-8 rounded-2xl border border-red-500/30 bg-red-500/10 p-5">
          <p className="flex gap-3 text-sm leading-6 text-red-300">
