    ...
    <Layout title="Analyze Image" subtitle="Upload a payment screenshot, scam message screenshot, or fake proof image for OCR-based analysis.">
      <div className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
        <form onSubmit={submit} className="rounded-2xl border border-slate-700/50 bg-panel p-6 shadow-soft">
          <label className="flex cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-primaryBlue/20 bg-primaryBlue/10 p-6 text-center transition hover:border-primaryBlue/30 hover:bg-primaryBlue/20">
            <ImageUp className="h-10 w-10 text-primaryBlue" />
            <span className="mt-3 font-semibold text-ink">{file ? file.name : 'Upload payment or scam screenshot'}</span>
            <span className="mt-1 text-sm text-slate-400">PNG, JPG, JPEG, or WEBP</span>
          ...
          {preview && (
            <div className="mt-5 overflow-hidden rounded-2xl border border-slate-700/50 bg-panel">
              <img src={preview} alt="Selected screenshot preview" className="max-h-96 w-full object-contain" />
            </div>
          )}

          <div className="mt-5 rounded-2xl border border-green-700/30 bg-green-700/10 p-4">
            <p className="flex items-center gap-2 font-semibold text-green-300"><Lightbulb className="h-4 w-4" /> Better OCR tips</p>
            <ul className="mt-3 space-y-2 text-sm leading-6 text-green-300">
          ...
          {loading && (
            <div className="mt-4 rounded-2xl border border-primaryBlue/20 bg-panel p-4">
              <Loader label={ocrStatus || 'Reading text from image...'} />
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-panel/10">
                <div className="h-full rounded-full bg-primaryBlue transition-all" style={{ width: `${ocrProgress}%` }} />
              </div>
              <p className="mt-2 text-right text-xs font-semibold text-primaryBlue">{ocrProgress}%</p>
            </div>
          )}
        </form>
        ...
        <div className="space-y-5">
          {ocrFallback && (
            <div className="rounded-2xl border border-amber-700/30 bg-amber-700/10 p-5">
              <div className="flex gap-3">
                <AlertTriangle className="mt-1 h-5 w-5 shrink-0 text-amber-700" />
                <div>
                  <h3 className="font-bold text-amber-300">Could not read text clearly</h3>
                  <p className="mt-2 text-sm leading-6 text-amber-300">Try uploading a clearer screenshot or paste the visible text manually.</p>
              ...
              <div className="mt-5 rounded-2xl border border-amber-700/30 bg-panel p-4">
          ...
          {!loading && !ocrFallback && !result && (
            <div className="rounded-2xl border border-slate-700/50 bg-panel p-6 text-center shadow-sm">
              <ImageUp className="mx-auto h-9 w-9 text-primaryBlue" />
              <h3 className="mt-3 font-semibold text-ink">Upload a screenshot to begin</h3>
              <p className="mt-2 text-sm leading-6 text-slate-400">FinShield will read text in your browser, then analyze the extracted content for fraud signals.</p>
            </div>
          )}
          ...
          {result?.ocr_text && (
            <div className="rounded-2xl border border-slate-700/50 bg-panel p-5 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h3 className="font-semibold text-ink">OCR extracted text</h3>
                {result.ocr_method && <span className="rounded-full bg-primaryBlue/10 px-3 py-1 text-xs font-semibold text-primaryBlue">Read by browser OCR</span>}
              </div>
              <p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-400">{result.ocr_text}</p>
            </div>
          )}
