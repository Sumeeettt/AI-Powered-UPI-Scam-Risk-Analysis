...
export default function RiskMeter({ risk_score = 0, risk_level = 'Safe', trust_score = null }) {
  ...
  return (
    <div className={`rounded-2xl border ${style.border} bg-panel p-5 shadow-sm`}>
      ...
      <div className="mt-5 h-3 overflow-hidden rounded-full bg-panel/10">
        <div className={`h-full ${style.bar} transition-all duration-700`} style={{ width: `${risk_score}%` }} />
      </div>
      {trustScore < 50 && (
        <p className="mt-4 rounded-lg border border-amber-700/30 bg-amber-700/10 p-3 text-sm font-semibold text-amber-300">
          Low trust score. Do not proceed without official verification.
        </p>
      )}
      {['High Risk', 'Critical'].includes(risk_level) && (
        <p className="mt-3 rounded-lg border border-red-500/30 bg-red-500/10 p-3 text-sm font-semibold text-red-300">
