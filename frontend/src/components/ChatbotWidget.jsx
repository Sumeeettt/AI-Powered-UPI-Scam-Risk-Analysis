...
export default function ChatbotWidget() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50">
      {open && (
        <div className="mb-3 flex h-[34rem] w-[calc(100vw-2.5rem)] max-w-sm flex-col overflow-hidden rounded-2xl border border-slate-700/50 bg-panel shadow-2xl">
          <div className="flex items-start justify-between gap-3 border-b border-slate-700/50 bg-panel p-4 text-ink">
            <div>
          ...
          <div className="flex-1 space-y-3 overflow-y-auto p-4">
            {messages.map((message, index) => (
              <div key={`${message.role}-${index}`} className={`rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === 'user' ? 'ml-8 bg-primaryBlue text-white' : 'mr-8 bg-panel text-slate-400'}`}>
                {message.text}
              </div>
            ))}
            {loading && <div className="mr-8 rounded-2xl bg-panel px-4 py-3 text-sm text-slate-400">Typing...</div>}
          </div>

          <div className="border-t border-slate-700/50 p-3">
            <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
              {suggestions.slice(0, 3).map((question) => (
                <button key={question} type="button" onClick={() => ask(question)} className="shrink-0 rounded-full border border-primaryBlue/20 bg-primaryBlue/10 px-3 py-1 text-xs font-semibold text-primaryBlue transition hover:bg-primaryBlue/20">
                  {question}
                </button>
              ))}
            </div>
            <form onSubmit={(event) => { event.preventDefault(); ask(input); }} className="flex gap-2">
              ...
              <button type="submit" className="rounded-lg bg-primaryBlue px-3 text-white transition hover:bg-primaryBlue/80" aria-label="Send message">
